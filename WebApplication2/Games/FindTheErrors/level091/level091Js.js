...
function correct001() {
    ...
    audio.play();
    board001.innerHTML += "<div id=mark001></div><h3 id='text1'>compilation error - no is not declared (should be s.no)</h3>";
    score001.innerHTML = a++;
    if (a > 0) {
        message002.innerHTML = "<button class=blue001 onclick=nextLevel()>Next Level</button>";
        board001.innerHTML = "<img src=../Capture20.PNG height=457 width=351 />";
        board001.innerHTML += "<div id=mark001></div><h3 id='text1'>compilation error - no is not declared (should be s.no)</h3>";
    }
}
...
function nextLevel() {
    window.location = "../level092/level092.html";
}